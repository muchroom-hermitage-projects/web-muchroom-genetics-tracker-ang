- [ ] Filters left panel should now also include "show contaminated", and "show clean" (uncontaminated) checkboxes.
- [ ] Relations should no longer be editable - the input should remains, but be disabled. Instead we should have defaults automatically selected based on the selected child "Type":
...
- [ ] Replace remove node alert window with a proper modal with "Yes" and "No" buttons
